...
uint64_t fastExponent(uint64_t a, uint64_t b ,uint64_t n){
    ...
    printf(YELLOW"a^b mod n: "GREEN"%lld.\n",ModRes);
    return ModRes;
...
bool MillerRabin(uint64_t n, uint64_t s ){
    uint64_t a;
    if (n < 2) {
        printf(YELLOW "%llu " RESET "is " RED "Composite.\n",n);
        return false;
    }
    if (n != 2 && n % 2==0) {
        printf(YELLOW "%llu " RESET "is " RED "Composite.\n",n);
    ...
    printf(YELLOW "%llu " RESET "is " GREEN "Prime.\n",n);
    return true;
}


int main(int argc, char *argv[]){
uint64_t opt; uint64_t AorS,N,B; 
bool fastExp;
while ((opt = getopt(argc, argv, "n:a:s:b:")) != -1){
        switch (opt){
        case 'n':
            N = strtol(optarg, NULL, 10);
            break;
        case 'a':
            Random_A = strtol(optarg, NULL, 10);
            break;
        case 's':
            AorS = strtol(optarg, NULL, 10);
            break;
         case 'b':
    ...
    if(fastExp){
        AorS = Random_A;
        fastExponent(AorS,B,N);
    }else{
         if(!AorS){ AorS = 10; }
         if(AorS){ printf("With" BLUE " %lld "RESET "'S' iteration... \n",AorS);}
         if(Random_A) printf("With" BLUE " %lld "RESET "'A' random number... \n",Random_A);
         else printf("Random " BLUE "'A'" RESET " will be picked by the program... \n");
         MillerRabin(N,AorS); 
    }
